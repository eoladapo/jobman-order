...
export const getOrdersByOrderId = async (orderId: string): Promise<IOrderDocument> => {
  const order: IOrderDocument[] = (await OrderModel.aggregate([{ $match: { orderId } }])) as IOrderDocument[];
  return order[0];
};

export const getOrdersBySellersId = async (sellerId: string): Promise<IOrderDocument[]> => {
...
export const approveDeliveryDate = async (orderId: string, data: IExtendedDelivery): Promise<IOrderDocument> => {
  ...
  if (order) {
    ...
    sendNotification(order, order.buyerUsername, `approved your order delivery date extension request.`);
  }
  return order;
};
...
export const rejectDeliveryDate = async (orderId: string): Promise<IOrderDocument> => {
  ...
  if (order) {
    ...
    sendNotification(order, order.buyerUsername, `rejected your order delivery date extension request.`);
  }
  return order;
};
