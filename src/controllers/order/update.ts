...
import { BadRequestError, IDeliveredWork, IOrderDocument, uploads } from '@eoladapo/jobman-shared';
...
const requestExtension = async (req: Request, res: Response): Promise<void> => {
  ...
  if (error?.details) {
    throw new BadRequestError(error.details[0].message, 'Update requestSchema() method');
  }
  const { orderId } = req.params;
