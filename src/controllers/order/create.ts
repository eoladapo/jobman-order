...
import { BadRequestError, IOrderDocument } from '@eoladapo/jobman-shared';
import { createOrder } from '@order/services/order.service';

const stripe: Stripe = new Stripe(config.STRIPE_API_KEY!, { typescript: true });
...
const intent = async (req: Request, res: Response): Promise<void> => {
  ...
  if (customerId) {
    ...
    paymentIntent = await stripe.paymentIntents.create({
      amount: Math.floor((req.body.price + serviceFee) * 100), // convert to cents
      currency: 'usd',
      customer: customerId,
    ...
    res
      ...
      .json({ message: 'Order intent created successfully', clientSecret: paymentIntent.client_secret, paymentIntentId: paymentIntent.id });
  }
};
